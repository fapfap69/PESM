...
PHP;
    ...
    private function generateConvertMethod(string $rule, array $info): string
    {
        ...
        // Special case for UnaryExpr
        if ($rule === 'UnaryExpr') {
            $code .= "        \$op = \$data['op']['text'];\n";
            $code .= "        \$expr = \$this->convert(\$data['expr']);\n";
            $code .= "        return new \\PESM\\Parser\\AST\\UnaryOpNode(\$op, \$expr);\n";
            $code .= "    }\n\n";
            return $code;
        }
        ...
        // Special case for Interrupt statements (InterruptWithVarStmt, InterruptSimpleStmt)
        if (in_array($rule, ['InterruptWithVarStmt', 'InterruptSimpleStmt'])) {
            $code .= "        \$args = \$this->extractInterruptArguments(\$data);\n";
            $code .= "        return new \\PESM\\Parser\\AST\\{$nodeClass}(...\$args);\n";
            $code .= "    }\n\n";
            return $code;
        }
        
        // Special case for FOREACH statements (ForeachInStmt, ForeachRangeStmt)
        if (in_array($rule, ['ForeachInStmt', 'ForeachRangeStmt'])) {
            $code .= "        \$args = \$this->extractForeachArguments(\$data, '$rule');\n";
            $code .= "        return new \\PESM\\Parser\\AST\\{$nodeClass}(...\$args);\n";
            $code .= "    }\n\n";
            return $code;
        }
        
        // Special case for SWITCH statement (SwitchStatement)
        if ($rule === 'SwitchStatement') {
            $code .= "        \$args = \$this->extractSwitchArguments(\$data);\n";
            $code .= "        return new \\PESM\\Parser\\AST\\{$nodeClass}(...\$args);\n";
            $code .= "    }\n\n";
            return $code;
        }
        ...
        // Special case for CommandDecl
        if ($rule === 'CommandDecl') {
            $code .= "        \$commands = [];\n";
            $code .= "        if (isset(\$data['head'])) {\n";
            $code .= "            \$headNode = \$this->convert(\$data['head']);\n";
            $code .= "            \$commands[] = \$headNode->name;\n";
            $code .= "        }\n";
            $code .= "        if (isset(\$data['tail']) && is_array(\$data['tail'])) {\n";
            $code .= "            foreach (\$data['tail'] as \$cmd) {\n";
            $code .= "                \$cmdNode = \$this->convert(\$cmd);\n";
            $code .= "                \$commands[] = \$cmdNode->name;\n";
            $code .= "            }\n";
            $code .= "        }\n";
            $code .= "        return new \\PESM\\Parser\\AST\\{$nodeClass}(\$commands);\n";
            $code .= "    }\n\n";
            return $code;
        }
